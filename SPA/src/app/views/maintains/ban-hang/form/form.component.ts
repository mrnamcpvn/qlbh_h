...
import { CustomerDebtInfo } from '@models/maintains/cong-no/cong-no';
...
export class FormComponent extends InjectBase implements OnInit, AfterViewInit, OnDestroy {
   ...
   * 2. Cộng lại số lượng đã lưu trong DB (originalChiTiet) - chỉ khi edit
   * 3. Trừ đi số lượng đang chọn trên form (listChiTiet)
   */
  private recalculateStock() {
    ...
    // Edit mode: cộng lại tồn đã xuất trong DB
    ...
    // Trừ đi số lượng đang chọn trên form
  ...
  save() {
    let warnings: string[] = [];
    if (this.debtInfo?.hasOverdueOrders) {
      warnings.push(`có ${this.debtInfo.overdueOrderCount} đơn quá hạn`);
    }
    if (this.debtInfo?.isOverLimit) {
      warnings.push(`đã vượt hạn mức công nợ (dư nợ: ${this.debtInfo.currentDebt.toLocaleString('vi-VN')}₫ / hạn mức: ${this.debtInfo.creditLimit.toLocaleString('vi-VN')}₫)`);
    }
    if (warnings.length > 0) {
      const msg = `KH ${warnings.join(' và ')}. Tiếp tục lưu?`;
      this.snotifyService.confirm(msg, 'Cảnh báo công nợ', () => this.doCreate());
    } else {
      this.doCreate();
    }
  }
  ...
  viewDebtDetail() {
    const khId = this.data.iD_KH;
    if (!khId) return;
    const kh = this.khachHangs.find(x => x.id === khId);
    const tenKH = kh ? kh.ten : '';
    const url = `/#/maintain/cong-no?khachHangID=${khId}&tenKH=${encodeURIComponent(tenKH)}`;
